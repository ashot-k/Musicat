...
const ProductPage = () => {
  const { id } = useParams();
  const { data: product, error, isPending } = useFetchData('http://localhost:8080/api/products/' + id);
  ;
        ...
        <div className="row">
          ...
          <div className="d-flex flex-column align-items-center col-md-6 w-100 text-start mb-4 gap-3">
            <div className="d-flex flex-row align-items-start gap-4 w-100">
              <div>
                {/* <img className="border border-secondary border-1 rounded product-image" src={{domain} + product.imageURL} alt="Product Image" /> */}
          ...
          style="display: grid; justify-content: center"
              <div class="d-flex flex-column align-items-center my-4"></div>
          */}
              </div>
            </div>
            ...
            {/* <div style={{ textAlign: 'center' }} className="my-2" th: insert="~{/fragments/track-listing :: track-list}">
        </div> */}
